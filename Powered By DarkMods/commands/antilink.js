...
export default {
    ...
    async execute({ sock, msg, args, groupSettings }) {
        ...
        const current = groupSettings.antilink_enabled;
        const shouldEnable = action === 'on' ? true : action === 'off' ? false : !current;

        await Database.updateGroupSettings(jid, { antilink_enabled: shouldEnable });
        await react(sock, msg, shouldEnable ? '✅' : '❌');

        return sendReply(sock, jid, shouldEnable ? '🟢 AntiLink ON' : '🔴 AntiLink OFF', { quoted: null });
    }
};