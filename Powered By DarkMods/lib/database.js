...
class JSONDatabase {
    ...
    getDefaultUserSettings() {
        return {
            prefix: '!',
            bot_name: '𝗟 𝗢 𝗩 𝗘 - 𝗫 𝗗 - 𝗕 𝗢 𝗧',
            bot_mode: 'private',
            language: 'en',
            antidelete_enabled: false,
            autoreact_enabled: false,
            menu_image: 'https://i.postimg.cc/JnFNPNKk/3c1ace542faf555082f09d1a15c9c4c2.jpg',
            welcome_image: 'https://i.postimg.cc/Gt3Wx1MW/6e958f635ab08d5f45c0c6573d3eb2de.jpg'
        };
    }
    ...
    async cleanupUserMedia(phoneNumber) {
        ...
        const keysToDelete = Object.keys(this.db.userMedia).filter(key => 
            this.db.userMedia[key].phone_number === phoneNumber
        );
        
        // Supprimer les fichiers physiques avant de supprimer les entrées
        ...
        this.saveDB();
        console.log(`🗑️ Media cleaned up for: ${phoneNumber} (${keysToDelete.length} items deleted)`);
        return true;
    }
    ...
    async deleteUserMedia(mediaName, mediaType) {
        // Correction : parcourir correctement les clés pour trouver la correspondance
        const keys = Object.keys(this.db.userMedia || {});
        let foundKey = null;
        
        for (const k of keys) {
            if (k.startsWith(`${mediaType}_${mediaName}`)) {
                foundKey = k;
                break;
            }
        }
        
        if (foundKey && this.db.userMedia[foundKey]) {
            // Supprimer le fichier physiquement
            const filePath = this.db.userMedia[foundKey].file_path;
            if (filePath && existsSync(filePath)) {
                try {
                    unlinkSync(filePath);
                } catch (e) {
            ...
            delete this.db.userMedia[foundKey];
            this.saveDB();
            return true;
        }
        return false;
    ...
    resetUserWarnings(jid, userId = null) {
        ...
        if (userId) {
            ...
            // Correction : collecter les clés avant suppression pour éviter les bugs d'itération
            const keysToDelete = Object.keys(this.db.userWarnings).filter(key => 
                key.startsWith(`${jid}_`)
            );
