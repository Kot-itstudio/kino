...
class SudoManager {
    ...
    loadSudoUsers() {
        try {
            if (!fs.existsSync(SUDO_FILE)) {
                ...
                fs.writeFileSync(SUDO_FILE, JSON.stringify(defaultData, null, 2), 'utf-8');
                return defaultData;
            }
            
            const data = fs.readFileSync(SUDO_FILE, 'utf-8');
            // Correction : gérer le cas où le fichier est vide
            if (!data.trim()) {
                const defaultData = { users: [] };
                fs.writeFileSync(SUDO_FILE, JSON.stringify(defaultData, null, 2), 'utf-8');
                return defaultData;
            ...
            // Correction : réinitialiser le fichier si le JSON est corrompu
            const defaultData = { users: [] };
            try {
                fs.writeFileSync(SUDO_FILE, JSON.stringify(defaultData, null, 2), 'utf-8');
            } catch (writeError) {
                console.error('Error resetting sudo file:', writeError);
            }
            return defaultData;
        }
    }
    ...
    saveSudoUsers() {
        try {
            fs.writeFileSync(SUDO_FILE, JSON.stringify(this.sudoUsers, null, 2), 'utf-8');
        } catch (error) {
